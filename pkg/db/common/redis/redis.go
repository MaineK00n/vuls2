...
// redis: HASH KEY: "<ecosystem>#detection#<Root ID>" FIELD: <Source ID> VALUE: criteriaTypes.Criteria
		...
		if err := func() error {
			...
			case dbTypes.SearchDetectionPkg:
				...
				for _, rootID := range rootIDs {
					...
					sm := make(map[sourceTypes.SourceID]criteriaTypes.Criteria)
					for k, v := range m {
						bs, err := v.AsBytes()
						if err != nil {
			...
			case dbTypes.SearchDetectionRoot:
				...
				ds, err := c.getDetection(ctx, queries[0])
				if err != nil {
					return errors.WithStack(err)
				}
				for _, d := range ds {
			...
			case dbTypes.SearchDetectionAdvisory:
				...
				am, err := c.getAdvisory(ctx, queries[0])
				if err != nil {
			...
			case dbTypes.SearchDetectionVulnerability:
				...
				vm, err := c.getVulnerability(ctx, queries[0])
				if err != nil {
...
func (c *Connection) GetVulnerabilityData(searchType dbTypes.SearchDataType, id string) (*dbTypes.VulnerabilityData, error) {
	...
	case dbTypes.SearchDataVulnerability:
		...
		m, err := c.getVulnerability(ctx, id)
		if err != nil {
			return nil, errors.Wrap(err, "get vulnerability")
		}
		if m == nil {
...
func (c *Connection) getDetection(ctx context.Context, rootID string) ([]dbTypes.VulnerabilityDataDetection, error) {
...
func (c *Connection) getAdvisory(ctx context.Context, id string) (map[sourceTypes.SourceID]map[string][]advisoryTypes.Advisory, error) {
	...
	am := make(map[sourceTypes.SourceID]map[string][]advisoryTypes.Advisory)
	for k, v := range m {
		...
		if am[sourceTypes.SourceID(ds)] == nil {
			am[sourceTypes.SourceID(ds)] = make(map[string][]advisoryTypes.Advisory)
		}
		am[sourceTypes.SourceID(ds)][rootID] = as
	}

	return am, nil
}

func (c *Connection) getVulnerability(ctx context.Context, id string) (map[sourceTypes.SourceID]map[string][]vulnerabilityTypes.Vulnerability, error) {
	...
	vm := make(map[sourceTypes.SourceID]map[string][]vulnerabilityTypes.Vulnerability)
	for k, v := range m {
		ds, rootID, ok := strings.Cut(k, "#")
		if !ok {
			return nil, errors.Errorf("unexpected field format. expected: %s, actual: %s -> %s", "<Source ID>#<Root ID>", fmt.Sprintf("vulnerability#advisory#%s", id), k)
		...
		if vm[sourceTypes.SourceID(ds)] == nil {
			vm[sourceTypes.SourceID(ds)] = make(map[string][]vulnerabilityTypes.Vulnerability)
		}
		vm[sourceTypes.SourceID(ds)][rootID] = vs
	}

	return vm, nil
}

func (c *Connection) getRoot(ctx context.Context, id string) (vulnerabilityRoot, error) {
...
func (c *Connection) putAdvisory(ctx context.Context, data dataTypes.Data) error {
	m := make(map[string][]advisoryTypes.Advisory)
	...
	for id, as := range m {
		...
		if err := c.conn.Do(ctx, c.conn.B().Hset().Key(fmt.Sprintf("vulnerability#advisory#%s", id)).FieldValue().FieldValue(fmt.Sprintf("%s#%s", data.DataSource, data.ID), string(bs)).Build()).Error(); err != nil {
			return errors.Wrapf(err, "HSET %s %s %q", fmt.Sprintf("vulnerability#advisory#%s", id), fmt.Sprintf("%s#%s", data.DataSource, data.ID), string(bs))
		}
	}
...
func (c *Connection) putVulnerability(ctx context.Context, data dataTypes.Data) error {
	...
	for id, vs := range m {
		...
		if err := c.conn.Do(ctx, c.conn.B().Hset().Key(fmt.Sprintf("vulnerability#vulnerability#%s", id)).FieldValue().FieldValue(fmt.Sprintf("%s#%s", data.DataSource, data.ID), string(bs)).Build()).Error(); err != nil {
			return errors.Wrapf(err, "HSET %s %s %q", fmt.Sprintf("vulnerability#vulnerability#%s", id), fmt.Sprintf("%s#%s", data.DataSource, data.ID), string(bs))
		}
	}
